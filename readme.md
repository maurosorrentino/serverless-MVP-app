api gateway (http) as is around 70% cheaper, we could put a policy that will allow a specific service to call the api
and set origin header to a specific domain, unless some rest feature is needed (for example for complicated API we should enable traces, it is usually fine to use http)
